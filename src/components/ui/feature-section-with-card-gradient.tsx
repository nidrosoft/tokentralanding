    ...
    <section className="bg-[#f8fafc] py-20 lg:py-28">
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        ...
        <div className="text-center mb-16">
          <span className="text-sm font-semibold uppercase tracking-wider text-blue-600 mb-3 block">
            The Problem
          </span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
          {problemCards.map((card) => (
            <div
              key={card.title}
              className="relative bg-gradient-to-b from-white to-slate-50 p-6 md:p-8 rounded-2xl overflow-hidden border border-slate-200 hover:shadow-lg hover:-translate-y-1 transition-all duration-300"
            >
              <Grid size={20} />
              ...
              {/* Stat */}
              <p className="text-3xl md:text-4xl font-bold text-slate-900 relative z-20 mb-2">
                {card.stat}
              </p>
              
              {/* Title */}
              <p className="text-lg font-semibold text-slate-800 relative z-20 mb-2">
                {card.title}
              </p>
              
              {/* Description */}
              <p className="text-slate-600 text-base relative z-20">
                {card.description}
              </p>
            </div>
          ))}
